...
export const getCertificateAuthorityFileUrls = () => {
  ...
  // are reused
  const applicationDirectoryUrl = getApplicationDirectoryUrl()
...
const getApplicationDirectoryUrl = () => {
  const { platform } = process

  if (platform === "darwin") {
    return resolveUrl(`./Library/Application Support/jsenv_https_localhost/`, process.env.HOME)
  }

  if (platform === "linux") {
    if (process.env.XDG_CONFIG_HOME) {
      return resolveUrl(`./jsenv_https_localhost/`, process.env.XDG_CONFIG_HOME)
    }
    return resolveUrl(`./.config/jsenv_https_localhost/`, process.env.HOME)
  }

  if (platform === "win32") {
    if (process.env.LOCALAPPDATA) {
      return resolveUrl(`./jsenv_https_localhost/`, process.env.LOCALAPPDATA)
    }
